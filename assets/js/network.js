 ...
 * Adapted to run inside .network-wrap rather than fullscreen.
 */
...
const container = document.getElementById('network-wrap');
if (!container) return;
...
const glowF = defs.append('filter').attr('id','net-glow')
...
const charge   = isMobile ? -200 : -300;

const sim = d3.forceSimulation(GRAPH.nodes)
  .force('link', d3.forceLink(GRAPH.links).id(d => d.id)
    .distance(d => linkDist - d.weight * (isMobile ? 0.4 : 0.65))
    .strength(0.6))
...
nodeG.append('circle').attr('class','net-node-glow-outer')
  .attr('r', d => nodeR(d) * 2.6)
  .attr('fill', d => hexToRgba(d.color, 0.07));

nodeG.append('circle').attr('class','net-node-glow-mid')
  .attr('r', d => nodeR(d) * 1.65)
  .attr('fill', d => hexToRgba(d.color, 0.16));

nodeG.append('circle').attr('class','net-node-core')
  .attr('r', d => nodeR(d))
  .attr('fill', d => d.color)
...
nodeG.append('text').attr('class','net-label')
...
const panelClose = document.getElementById('net-panel-close');
const panelDot   = document.getElementById('net-panel-dot');
const panelName  = document.getElementById('net-panel-name');
const panelCount = document.getElementById('net-panel-count');
const panelList  = document.getElementById('net-panel-list');
...
function openPanel(d) {
  nodeG.selectAll('.net-node-core').classed('active', n => n.id === d.id);
...
function closePanel() {
  ...
  nodeG.selectAll('.net-node-core').classed('active', false);
}
